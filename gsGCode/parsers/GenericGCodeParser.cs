...
namespace gs
{
    public class GenericGCodeParser
    {
        ...
        public GCodeFile Parse(TextReader input, bool excludeBlanks)
        {
            ...
            while ( input.Peek() >= 0 ) {
                ...
                if (excludeBlanks && l.type == GCodeLine.LType.Blank)
                {
                    continue;
                }
		...
		virtual protected bool parse_value_param(string token, int split, int skip, ref GCodeParam param)
		{
			...
			try {
				...
				if (numType == GCodeUtil.NumberType.Decimal) {
					...
					param.doubleValue = double.Parse(value, CultureInfo.InvariantCulture);
					return true;
